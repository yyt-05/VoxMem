...
1. 打开页面，进入语音工作台，确认显示 API 已连接。
2. 选择原声模式，点击开始录音，说一句短文本，停止后确认输入框出现识别结果。
3. 手动把识别错误改正确，例如把“张力”改成“张立”，点击确认或让输入框自动提交。
4. 再输入包含“张力”的内容，观察本地记忆自动替换为“张立”。
5. 切换轻整理模式，说“今天是星期一，不对，今天是星期二，下午三点同步需求”，观察整理后的最终文本。
6. 打开热词管理或输入质量观察，展示系统记录的纠错记忆和质量指标。
